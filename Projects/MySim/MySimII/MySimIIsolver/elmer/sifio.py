...
class Builder(object):
    ...
    def addSolver(self, body, solverSection):
        ...
        if "Heat+CFD" in solverSection["Equation"]:
            section["Convection"] = "computed"
...
class Sif(object):
    ...
    def _writeHeader(self, stream):
        ...
        self._writeAttr2(self._MESHDB_ATTR, self.meshPath, ".", stream)
        stream.write(_NEWLINE)
        if self.incPath:
    ...
    def _writeAttr2(self, name, value, value2, stream):
        stream.write(_INDENT)
        stream.write(name)
        stream.write(_WHITESPACE)
        stream.write('"%s"' % value)
        stream.write(_WHITESPACE)
        stream.write('"%s"' % value2)


class VariableAttr(object):

    def __init__(self, variables_str, functions_str):
        if (not variables_str) or variables_str.isspace():
            self.variables = "Time, Coordinate 1, Coordinate 2, Coordinate 3"
            self.functions = "t=tx(0); x=tx(1); y=tx(2); z=tx(3); " + functions_str
        else:
            self.variables = variables_str
            self.functions = functions_str

class VarArrayAttr(object):

    def __init__(self, variables_str, FunctionList):
        self.FunctionList = []
        if (not variables_str) or variables_str.isspace():
            self.variables = "Time, Coordinate 1, Coordinate 2, Coordinate 3"
            for item in FunctionList:
                self.FunctionList.append("t=tx(0); x=tx(1); y=tx(2); z=tx(3); " + item)
        else:
            self.variables = variables_str
            for item in FunctionList:
                self.FunctionList.append(item)
...
class _Writer(object):
    ...
    def _writeAttribute(self, key, data):
        ...
        elif isinstance(data, VariableAttr):
            self._stream.write(_NEWLINE)
            self._writeVariableAttr(key, data)
        elif isinstance(data, EqualAttr):
            self._stream.write(_NEWLINE)
            self._writeEqualAttr(key, data)
        elif isinstance(data, FArrayAttr):
            self._stream.write(_NEWLINE)
            self._writeFArrayAttr(key, data)
        elif isinstance(data, VarArrayAttr):
            self._stream.write(_NEWLINE)
            self._writeVarArrayAttr(key, data)
    ...
    def _writeEqualAttr(self, key, data):
        vals = []
        for val in data.split(","):
            vals.append(val)
        for i in range(len(vals)):
            if i > 0:
                self._stream.write(_NEWLINE)
            self._stream.write(_INDENT)
            self._stream.write(key + " %d"%(i+1))
            self._stream.write(" = Equals ")
            self._stream.write(vals[i])

    def _writeFArrayAttr(self, key, data):
        for i in range(len(data)):
            if data[i] == FLOAT_BLANK_MARK:
                continue
            if i > 0:
                self._stream.write(_NEWLINE)
            self._stream.write(_INDENT)
            self._stream.write(key + " %d"%(i+1))
            self._stream.write(" = Real ")
            self._stream.write(str(data[i]))

    def _writeVarArrayAttr(self, key, data):
        for i in range(len(data.FunctionList)):
            text = data.FunctionList[i]
            if( i > 0):
                self._stream.write(_NEWLINE)
            self._stream.write(_INDENT)
            self._stream.write(key + " %d"%(i+1))
            self._stream.write(" = Variable ")
            self._stream.write(data.variables)
            self._stream.write(_NEWLINE)
            self._stream.write(_INDENT)
            self._stream.write("Real MATC ")
            self._stream.write('"%s"' % (text) )

    def _writeVariableAttr(self, key, data):
        self._stream.write(_INDENT)
        self._stream.write(key)
        self._stream.write(" = Variable ")
        self._stream.write(data.variables)
        ...
        self._stream.write(_INDENT)
        self._stream.write("Real MATC ")
        self._stream.write('"%s"' % (data.functions) )
