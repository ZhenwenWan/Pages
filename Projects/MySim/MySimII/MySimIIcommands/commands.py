...
class _Initiatives(CommandManager):
    "The MySimII_Initiatives command definition"

    def __init__(self):
        super(_Initiatives, self).__init__()
        self.resources = {
            "Pixmap":   "materials.svg",
            "MenuText": QtCore.QT_TRANSLATE_NOOP("MySimII_Initiatives","Initiatives"),
            "Accel":    "N, A",
            "ToolTip":  QtCore.QT_TRANSLATE_NOOP("MySimII_Initiatives","Initiatives")}
        self.is_active = "with_document"

    def Activated(self):
        FreeCAD.ActiveDocument.openTransaction("Create Initiatives")
        obj = FreeCAD.ActiveDocument.addObject("MySimII::FeaturePython", "Initiative")
        from MySimIIObjects import _MySimIIInitiatives
        _MySimIIInitiatives._MySimIIInitiatives(obj)
        if FreeCAD.GuiUp:
            from MySimIIGuiObjects import _ViewProviderMySimIIInitiatives
            _ViewProviderMySimIIInitiatives._ViewProviderMySimIIInitiatives(obj.ViewObject)
        FreeCADGui.ActiveDocument.setEdit(obj.Name)
        FreeCADGui.Selection.clearSelection()
        FreeCAD.ActiveDocument.recompute()

class _Forces(CommandManager):
    "The MySimII_Forces command definition"

    def __init__(self):
        super(_Forces, self).__init__()
        self.resources = {
            "Pixmap":   "materials.svg",
            "MenuText": QtCore.QT_TRANSLATE_NOOP("MySimII_Forces","Forcings"),
            "Accel":    "N, A",
            "ToolTip":  QtCore.QT_TRANSLATE_NOOP("MySimII_Forces","Forcings")}
        self.is_active = "with_document"

    def Activated(self):
        FreeCAD.ActiveDocument.openTransaction("Create Forces")
        obj = FreeCAD.ActiveDocument.addObject("MySimII::FeaturePython", "Forcing")
        from MySimIIObjects import _MySimIIForces
        _MySimIIForces._MySimIIForces(obj)
        if FreeCAD.GuiUp:
            from MySimIIGuiObjects import _ViewProviderMySimIIForces
            _ViewProviderMySimIIForces._ViewProviderMySimIIForces(obj.ViewObject)
        FreeCADGui.ActiveDocument.setEdit(obj.Name)
        FreeCADGui.Selection.clearSelection()
        FreeCAD.ActiveDocument.recompute()

class _Materials(CommandManager):
    "The MySimII_Materials command definition"

    def __init__(self):
        super(_Materials, self).__init__()
        self.resources = {
            "Pixmap":   "materials.svg",
            "MenuText": QtCore.QT_TRANSLATE_NOOP("MySimII_Materials","Materials"),
            "Accel":    "N, A",
            "ToolTip":  QtCore.QT_TRANSLATE_NOOP("MySimII_Materials","Materials")}
        self.is_active = "with_document"

    def Activated(self):
        FreeCAD.ActiveDocument.openTransaction("Create Material")
        obj = FreeCAD.ActiveDocument.addObject("MySimII::FeaturePython", "Material")
        from MySimIIObjects import _MySimIIMaterials
        _MySimIIMaterials._MySimIIMaterials(obj)
        if FreeCAD.GuiUp:
            from MySimIIGuiObjects import _ViewProviderMySimIIMaterials
            _ViewProviderMySimIIMaterials._ViewProviderMySimIIMaterials(obj.ViewObject)
        FreeCADGui.ActiveDocument.setEdit(obj.Name)
        FreeCADGui.Selection.clearSelection()
        FreeCAD.ActiveDocument.recompute()

class _Boundaries(CommandManager):
    "The MySimII_Boundaries command definition"

    def __init__(self):
        super(_Boundaries, self).__init__()
        self.resources = {
            "Pixmap":   "materials.svg",
            "MenuText": QtCore.QT_TRANSLATE_NOOP("MySimII_Boundaries","Boundaries"),
            "Accel":    "N, A",
            "ToolTip":  QtCore.QT_TRANSLATE_NOOP("MySimII_Boundaries","Boundaries")}
        self.is_active = "with_document"

    def Activated(self):
        FreeCAD.ActiveDocument.openTransaction("Create Boundary")
        obj = FreeCAD.ActiveDocument.addObject("MySimII::FeaturePython", "Boundary")
        from MySimIIObjects import _MySimIIBoundaries
        _MySimIIBoundaries._MySimIIBoundaries(obj)
        sel = FreeCADGui.Selection.getSelection()
        if len(sel) == 1 and sel[0].isDerivedFrom("Fem::FemMeshObjectPython"):
            selobj = sel[0]
            tmplist = selobj.MeshGroupList
            tmplist.append(obj)
            selobj.MeshGoupList = tmplist
        if FreeCAD.GuiUp:
            from MySimIIGuiObjects import _ViewProviderMySimIIBoundaries
            _ViewProviderMySimIIBoundaries._ViewProviderMySimIIBoundaries(obj.ViewObject)
        FreeCADGui.ActiveDocument.setEdit(obj.Name)
        FreeCADGui.Selection.clearSelection()
        FreeCAD.ActiveDocument.recompute()

class _MeshBoundaries(CommandManager):
    "The MySimII_MeshBoundaries command definition"

    def __init__(self):
        super(_MeshBoundaries, self).__init__()
        self.resources = {
            "Pixmap":   "materials.svg",
            "MenuText": QtCore.QT_TRANSLATE_NOOP("MySimII_MeshBoundaries","VaryingSize"),
            "Accel":    "N, A",
            "ToolTip":  QtCore.QT_TRANSLATE_NOOP("MySimII_MeshBoundaries","VaryingSize")}
        self.is_active = "always"

    def Activated(self):
        FreeCAD.ActiveDocument.openTransaction("Create MeshBoundary")
        obj = FreeCAD.ActiveDocument.addObject("MySimII::FeaturePython", "MeshVarying")
        from MySimIIObjects import _MySimIIMeshBoundaries
        _MySimIIMeshBoundaries._MySimIIMeshBoundaries(obj)
        sel = FreeCADGui.Selection.getSelection()
        if len(sel) == 1 and sel[0].isDerivedFrom("Fem::FemMeshObjectPython"):
            selobj = sel[0]
            tmplist = selobj.MeshBoundaryLayerList
            tmplist.append(obj)
            selobj.MeshBoundaryLayerList = tmplist
        if FreeCAD.GuiUp:
            from MySimIIGuiObjects import _ViewProviderMySimIIMeshBoundaries
            _ViewProviderMySimIIMeshBoundaries._ViewProviderMySimIIMeshBoundaries(obj.ViewObject)
        FreeCADGui.ActiveDocument.setEdit(obj.Name)
        FreeCADGui.Selection.clearSelection()
        FreeCAD.ActiveDocument.recompute()

class _MeshRegions(CommandManager):
    "The MySimII_MeshRegions command definition"

    def __init__(self):
        super(_MeshRegions, self).__init__()
        self.resources = {
            "Pixmap":   "materials.svg",
            "MenuText": QtCore.QT_TRANSLATE_NOOP("MySimII_MeshRegions","FixedSize"),
            "Accel":    "N, A",
            "ToolTip":  QtCore.QT_TRANSLATE_NOOP("MySimII_MeshRegions","FixedSize")}
        self.is_active = "always"

    def Activated(self):
        FreeCAD.ActiveDocument.openTransaction("Create MeshRegion")
        obj = FreeCAD.ActiveDocument.addObject("MySimII::FeaturePython", "MeshFixed")
        from MySimIIObjects import _MySimIIMeshRegions
        _MySimIIMeshRegions._MySimIIMeshRegions(obj)
        sel = FreeCADGui.Selection.getSelection()
        if len(sel) == 1 and sel[0].isDerivedFrom("Fem::FemMeshObjectPython"):
            selobj = sel[0]
            tmplist = selobj.MeshRegionList
            tmplist.append(obj)
            selobj.MeshRegionList = tmplist
        if FreeCAD.GuiUp:
            from MySimIIGuiObjects import _ViewProviderMySimIIMeshRegions
            _ViewProviderMySimIIMeshRegions._ViewProviderMySimIIMeshRegions(obj.ViewObject)
        FreeCADGui.ActiveDocument.setEdit(obj.Name)
        FreeCADGui.Selection.clearSelection()
        FreeCAD.ActiveDocument.recompute()

class _MeshCreate(CommandManager):
    "The MySimII_MeshCreate command definition"

    def __init__(self):
        super(_MeshCreate, self).__init__()
        self.resources = {
            "Pixmap":   "materials.svg",
            "MenuText": QtCore.QT_TRANSLATE_NOOP("MySimII_MeshCreate","Create&View"),
            "Accel":    "N, A",
            "ToolTip":  QtCore.QT_TRANSLATE_NOOP("MySimII_MeshCreate","Create&View")}
        self.is_active = "always"

    def Activated(self):
        FreeCAD.ActiveDocument.openTransaction("Create MeshCreate")
        #obj = FreeCAD.ActiveDocument.addObject("MySimII::MySimIIMeshCreateObjectPython", "MeshCreate")
        obj = FreeCAD.ActiveDocument.addObject("Fem::FemMeshObjectPython", "MeshCreate")
        from MySimIIObjects import _MySimIIMeshCreate
        _MySimIIMeshCreate._MySimIIMeshCreate(obj)
        if FreeCAD.GuiUp:
            from MySimIIGuiObjects import _ViewProviderMySimIIMeshCreate
            _ViewProviderMySimIIMeshCreate._ViewProviderMySimIIMeshCreate(obj.ViewObject)
        FreeCADGui.ActiveDocument.setEdit(obj.Name)
        FreeCADGui.Selection.clearSelection()
        FreeCAD.ActiveDocument.recompute()

class _Solvers(CommandManager):
    "The MySimII_Solvers command definition"

    def __init__(self):
        super(_Solvers, self).__init__()
        self.resources = {
            "Pixmap":   "materials.svg",
            "MenuText": QtCore.QT_TRANSLATE_NOOP("MySimII_Solvers","Solver"),
            "Accel":    "N, A",
            "ToolTip":  QtCore.QT_TRANSLATE_NOOP("MySimII_Solvers","Solver")}
        self.is_active = "always"

    def Activated(self):
        FreeCAD.ActiveDocument.openTransaction("Create Solver")
        obj = FreeCAD.ActiveDocument.addObject("Fem::FemSolverObjectPython", "Solver")
        from MySimIIObjects import _MySimIISolvers
        _MySimIISolvers._MySimIISolvers(obj)
        if FreeCAD.GuiUp:
            from MySimIIGuiObjects import _ViewProviderMySimIISolvers
            _ViewProviderMySimIISolvers._ViewProviderMySimIISolvers(obj.ViewObject)
        FreeCADGui.ActiveDocument.setEdit(obj.Name)
        FreeCADGui.Selection.clearSelection()
        FreeCAD.ActiveDocument.recompute()

class _Simulation(CommandManager):
    "The MySimII_Simulation command definition"

    def __init__(self):
        super(_Simulation, self).__init__()
        self.resources = {
            "Pixmap":   "MySimII-Simulation.svg",
            "MenuText": QtCore.QT_TRANSLATE_NOOP("MySimII_Simulation","Simulation"),
            "Accel":    "N, A",
            "ToolTip":  QtCore.QT_TRANSLATE_NOOP("MySimII_Simulation","Simulation")}
        self.is_active = "always"

    def Activated(self):
        FreeCAD.ActiveDocument.openTransaction("Create Simulation")
        obj = FreeCAD.ActiveDocument.addObject("Fem::FemSolverObjectPython", "Simulation")
        from MySimIIObjects import _MySimIISimulation
        _MySimIISimulation._MySimIISimulation(obj)
        if FreeCAD.GuiUp:
            from MySimIIGuiObjects import _ViewProviderMySimIISimulation
            _ViewProviderMySimIISimulation._ViewProviderMySimIISimulation(obj.ViewObject)
        FreeCADGui.ActiveDocument.setEdit(obj.Name)
        FreeCADGui.Selection.clearSelection()
        FreeCAD.ActiveDocument.recompute()

class _LocalRun(CommandManager):
    "The MySimII_LocalRun command definition"

    def __init__(self):
        super(_LocalRun, self).__init__()
        self.resources = {
            "Pixmap":   "materials.svg",
            "MenuText": QtCore.QT_TRANSLATE_NOOP("MySimII_LocalRun","LocalRun"),
            "Accel":    "N, A",
            "ToolTip":  QtCore.QT_TRANSLATE_NOOP("MySimII_LocalRun","LocalRun")}
        self.is_active = "always"

    def Activated(self):
        FreeCAD.ActiveDocument.openTransaction("Create LocalRun")
        obj = FreeCAD.ActiveDocument.addObject("MySimII::MySimIILocalRunObjectPython", "LocalRun")
        from MySimIIObjects import _MySimIILocalRun
        _MySimIILocalRun._MySimIILocalRun(obj)
        if FreeCAD.GuiUp:
            from MySimIIGuiObjects import _ViewProviderMySimIILocalRun
            _ViewProviderMySimIILocalRun._ViewProviderMySimIILocalRun(obj.ViewObject)
        FreeCADGui.ActiveDocument.setEdit(obj.Name)
        FreeCADGui.Selection.clearSelection()
        FreeCAD.ActiveDocument.recompute()
