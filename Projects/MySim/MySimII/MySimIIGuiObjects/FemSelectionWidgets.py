...
def GeoIdentical(b1,b2): 
    if not b1.ShapeType == b2.ShapeType:
        FreeCAD.Console.PrintError('Error: different ShapeType \n')
        return False
    bb1 = b1.BoundBox
    bb2 = b2.BoundBox
    xbb1 = bb1.XLength
    ...
    ybb2 = bb2.YLength
    zbb2 = bb2.ZLength
    tolerance = pow(xbb1*ybb1*zbb1,0.333333)/10000
    if ( abs(xbb1 - xbb2) > tolerance  or
         abs(ybb1 - ybb2) > tolerance  or
         abs(zbb1 - zbb2) > tolerance  or
         ( len(b1.Vertexes) != len(b2.Vertexes)) ):
        return False
    for vertex1 in b1.Vertexes:
        dis0 = xbb1 + ybb1 + zbb1
        for vertex2 in b2.Vertexes:
            dis = vertex1.distToShape(vertex2)[0] 
            if dis < dis0:
                dis0 = dis
        if dis0 > tolerance:
            return False
...
def CorrespondToModel(obj, sub):
    if str(obj.Label).startswith("Model"):
        return (obj, sub)
    else:
        ele = obj.Shape.getElement(sub)
        for model in FreeCAD.ActiveDocument.Objects:
            if model.isDerivedFrom("Part::Feature"):
                if str(model.Label).startswith("Model"):
                    if ele.ShapeType == 'Solid':
                        for i, solid in enumerate(model.Shape.Solids):
                            if GeoIdentical(solid,ele):
                                sub = 'Solid' + str(i+1)
                                obj = model
                                return (model, sub)
                    if ele.ShapeType == 'Face':
                        for i, face in enumerate(model.Shape.Faces):
                            if GeoIdentical(face,ele):
                                sub = 'Face' + str(i+1)
                                obj = model
                                return (model, sub)
                    if ele.ShapeType == 'Edge':
                        for i, edge in enumerate(model.Shape.Edges):
                            if GeoIdentical(edge,ele):
                                sub = 'Edge' + str(i+1)
                                obj = model
                                return (model, sub)
        FreeCAD.Console.PrintError('Failed to correspond to Model \n')
...
class FemSelectionObserver:
    ...
    def addSelection(self, docName, objName, sub, pos):
        ...
        (model, ele) = CorrespondToModel( selected_object, sub )
        self.added_obj = ( model, ele )
